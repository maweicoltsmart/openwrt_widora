...
void si446x_part_info(void)
{
    ...
    printk("0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x \r\n",Pro2Cmd[0],Pro2Cmd[1],Pro2Cmd[2],Pro2Cmd[3],Pro2Cmd[4],Pro2Cmd[5],Pro2Cmd[6],Pro2Cmd[7]);
}
 ...
 * @param PROTOCOL    0 = Packet format is generic, no dynamic reprogramming of packet handler properties.
                      ...
                      PKT_CRC_CONFIG, CRC_ENDIAN/bool_ORDER in PKT_CONFG1 for TX and RX, PKT_FIELD_1_CRC_CONFIG for RX.
                      Other applicable properties in the packet handler group still need to be programmed. Field 1 should
                      have the length of 16 bools to contain the PHR with PKT_LEN_FIELD_SOURCE set to 1 for RX. PSDU field
