...
open_dev:
    ...
    {
        stRadioCfg.freq_rx[i] = ( uint32_t )( ( double )(428200000 + i * 200000) / ( double )FREQ_STEP );
    }
    for( i = 0; i < CN433_MAX_NB_CHANNELS; i++ )
    {
        stRadioCfg.freq_tx[i] = ( uint32_t )( ( double )(428200000 + 24 * 200000 + ( i ) * 200000) / ( double )FREQ_STEP );
    }
